# Performance Media

Drop solver performance visuals here before the README is updated.

Current assets:

- `motion-sketch-path-optimization.mp4`
- `duik-walkcycle-solve-unparent.mp4`
- `ik-expression-driven-path-optimization-stable-topology.mp4`

README preview attachment URLs:

- Motion sketch cleanup: `https://github.com/user-attachments/assets/cecca4cc-0578-474b-ae6f-c95c3fa3f359`
- Rig bake + unparent: `https://github.com/user-attachments/assets/370c4f05-5af7-484e-be8e-b9339c49ae47`
- Expression-driven path optimization: `https://github.com/user-attachments/assets/3a7fe26c-254a-4563-af44-04e7b533b732`

Recommended use:

- Use `.gif` or `.webp` for short inline README before/after clips.
- Use `.mp4` for higher-quality demos that should be linked from the README or attached to a GitHub Release.
- Keep filenames descriptive and stable, for example:
  - `timeline-dense-to-sparse.gif`
  - `motion-path-smoothing-before-after.mp4`
  - `shape-path-vertex-reduction.webp`

Keep source footage license-clean and avoid project-internal branding unless it
is meant to appear in the public `bbsolver` repository.
